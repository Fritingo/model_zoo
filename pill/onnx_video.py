...
session = ort.InferenceSession(w, sess_opt, providers=providers)
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        # print(outputs)
        all_t.append(time.time()-pt)
        # print(sum(all_t)/(i+1))
        image = frame.copy()
        # print(outputs)
        cv2.putText(image,'FPS: '+str(int(1/(all_t[-1]))),(30,30),cv2.FONT_HERSHEY_SIMPLEX,0.75,[225, 0, 0],thickness=2)
        ...
        cv2.imshow('frame', cv2.resize(image, None, fx=1.2, fy=1.2))
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    print(all_t)
    print(sum(all_t[10:])/300.0, 1/ (sum(all_t[10:])/300.0))
